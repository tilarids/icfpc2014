...
//        CT bottom = getMapItem(map, y+1, x);
    ...
    private Tuple<AIState, Function2<AIState, WorldState, Tuple<AIState, Integer>>> entryFactual(WorldState ws) {
        AIState initialState = createInitialState(ws);
...
//            }
            ...
            direction = isWalkable3(worldState.map, location.x, location.y - 1) == 1 && aistate.lastDirection != Direction.DOWN ?
                    ...
                    isWalkable3(worldState.map, location.x + 1, location.y) == 1 && aistate.lastDirection != Direction.LEFT ?
                            ...
                            isWalkable3(worldState.map, location.x, location.y + 1) == 1 && aistate.lastDirection != Direction.UP ?
                                    Direction.DOWN :
                                    Direction.LEFT;
            retval = new Tuple<>(new AIState(aistate.parsedStaticMap, direction, aistate.ghostStartPoints), direction);
    ...
    @Compiled
    private AIState createInitialState(WorldState ws) {
        ListCons<ListCons<Integer>> map = ws.map;
        return new AIState(parseStaticMap(map), 0, map(ws.ghosts, s -> s.location));
    }
...
//        System.out.println("sorted:" + sorted_map_walk(new_map, null, (ListCons<Integer> acc, Tuple<Integer, Integer> x) -> cons(x.a, acc)));
    ...
    static class GHCState {
        ...
        GHCState(GhostState ghostState, SortedMap<Integer> regs, SortedMap<Integer> data, Point ghostInitPos) {
            this.ghostState = ghostState;
            this.regs = regs;
            this.data = data;
            this.ghostInitPos = ghostInitPos;
        }
    }
    ...
    public GHCState ghcstate_write_val(GHCState state, Cons arg_cons, Integer val) {
        ...
        return
                arg_tag == 0 ? new GHCState(state.ghostState, sorted_map_assoc(state.regs, (Integer) second(arg_cons), val), state.data, state.ghostInitPos)
                        : arg_tag == 1 ? new GHCState(state.ghostState, sorted_map_assoc(state.regs, sorted_map_get(state.regs, (Integer) second(arg_cons), 0), val), state.data, state.ghostInitPos)
                        : new GHCState(state.ghostState, state.regs, sorted_map_assoc(state.data, (Integer) second(arg_cons), val), state.ghostInitPos);
    }
    ...
    public GHCState processGhostInfoRequest(WorldState world, GHCState state, Integer index, int requestType) {
        ...
        Point startPos = state.ghostInitPos;
        return
                4 == requestType ? new GHCState(gs, sorted_map_assoc(sorted_map_assoc(state.regs, 0, startPos.x), 1, startPos.y), state.data, state.ghostInitPos)
                        : 5 == requestType ? new GHCState(gs, sorted_map_assoc(sorted_map_assoc(state.regs, 0, gs.location.x), 1, gs.location.y), state.data, state.ghostInitPos)
                        : new GHCState(gs, sorted_map_assoc(sorted_map_assoc(state.regs, 0, gs.vitality), 1, gs.direction), state.data, state.ghostInitPos);
    }
    ...
    public GHCState processGhostInt(WorldState world, GHCState state, Integer num, Cons arg) {
        ...
        return
                0 == num ? new GHCState(new GhostState(gs.vitality, gs.location, sorted_map_get(state.regs, 0, 0)), state.regs, state.data, state.ghostInitPos)
                        : 1 == num ? new GHCState(gs, sorted_map_assoc(sorted_map_assoc(state.regs, 0, world.lambdaManState.location.x), 1, world.lambdaManState.location.y), state.data, state.ghostInitPos)
                        : 2 == num ? new GHCState(gs, sorted_map_assoc(sorted_map_assoc(state.regs, 0, world.lambdaManState.location.x), 1, world.lambdaManState.location.y), state.data, state.ghostInitPos)
                        : 3 == num ? new GHCState(gs, sorted_map_assoc(state.regs, 0, currentGhostIndex), state.data, state.ghostInitPos)
                        ...
                        : 7 == num ? new GHCState(gs, sorted_map_assoc(state.regs, 0, getMapItem(world.map, sorted_map_get(state.regs, 0, 0), sorted_map_get(state.regs, 1, 0))), state.data, state.ghostInitPos)
    ...
    public Integer runGhostStep(SortedMap<Cons> prog, WorldState world, int lev, GHCState state, Cons step) {
        ...
        GHCState inc_pc = new GHCState(state.ghostState, sorted_map_assoc(state.regs, 8, sorted_map_get(state.regs, 8, 0) + 1), state.data, state.ghostInitPos);
        return
                GHCOps.MOV == opcode ? runGhost(prog, world, lev, ghcstate_assoc(inc_pc, first(args), head(second(args))))
                        ...
                        ghcstate_read_val(state, first(tail(args))) < ghcstate_read_val(state, second(tail(args)))
                                ? new GHCState(state.ghostState, sorted_map_assoc(state.regs, 8, (Integer) first(args)), state.data, state.ghostInitPos)
                                : state)
    ...
    @Compiled
    public Integer getGhostDirection(WorldState world, ListCons<Cons> spec, Point ghostInitPos) {
        ...
        return runGhost(prog.b,
                world,
                0,
                new GHCState(ghostState, new SortedMap<Integer>(null, 0), new SortedMap<Integer>(null, 0), ghostInitPos));
    }
    ...
    public static void main(String[] args) {
        ...
        if (false) {
            ...
            System.out.println("direction:" + new Sample1().getGhostDirection(worldState, first(spec), new Point(5, 5)));
            System.exit(0);
        }
